...
import { cartPayment, checkOrder, getCardDetail } from "../Service";
...
const keys = ['Product Id', 'Product Name', 'Price', 'Quantity', 'Total Price']
...
export default function PayBill() {
    ...
    if (!Array.isArray(responseData) || responseData.length === 0) {
        return <NoResult pageName={'Pay Bill'} title={'No orders to be paid'} />
    }
    return (
        <Box
            sx={{
        ...
        >
            ...
            <div style={{ width: '100%', display: "flex", flexDirection: "row", justifyContent: "space-between", marginTop: 30, border: '2px solid #000', borderRadius: 10, flexDirection: 'column',overflow:'hidden' }}>
                    ...
                    <div style={{ width: '100%', display: "flex", flexDirection: "column",height:350,overflow:'scroll' }}>
                        {
                            responseData.map((item, idx) => {
                                return <div style={{ display: "flex", flexDirection: "row", marginTop: 10, justifyContent: "space-around", width: '100%',borderBottom:'1px solid #ccc',paddingBottom:10 }}>
                                    <span style={{ flex:1,textAlign: "center"}}>{item.product_id}</span>
                                    ...
                                    <span style={{ flex: 1, textAlign: "center" }}>${item.total_price}</span>
                                </div>
                            })
                        }
                    </div>
            ...
            <div>
                <Typography variant="h5" gutterBottom style={{ fontWeight: 600, marginTop: 10 }}>
